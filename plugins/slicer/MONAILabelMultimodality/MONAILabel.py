...
class MONAILabel(ScriptedLoadableModule):
    def __init__(self, parent):
        ...
        self.parent.title = "MONAILabel Multimodality"
...
"""
    ...
    def initializeAfterStartup(self):
        if not slicer.app.commandOptions().noMainWindow:
            ...
            slicer.app.settingsDialog().addPanel("MONAI Label Multimodality", self.settingsPanel)
...
class MONAILabelWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
    ...
    def onNextSampleButton(self):
        ...
        if len(self._volumeNodes) == 2:
            # exactly two volumes, show as three over three
            slicer.app.layoutManager().setLayout(slicer.vtkMRMLLayoutNode.SlicerLayoutThreeOverThreeView)
            # set the first three views to show the first volume, the second three views to show the second volume
            for idx, node in enumerate(slicer.util.getNodesByClass('vtkMRMLSliceCompositeNode')):
                node.SetBackgroundVolumeID(self._volumeNodes[0].GetID())
                if idx >= 3:
                    # actually show the second volume as foreground so annotation always happens in the first volume
                    node.SetForegroundVolumeID(self._volumeNodes[1].GetID())
                    node.SetForegroundOpacity(1.0)
            ...
            for node in slicer.util.getNodesByClass('vtkMRMLSliceNode'):
                # make nodes linkable in the same view group
                node.SetViewGroup(0)
